...
- `Model` — base abstract class: holds an `id` and requires `toMap()` and
...
p.setCoins(42).save(repo);

// find by id
PlayerData loaded = EzModel.find(repo, "uuid-1234");

// query
com.skyblockexp.ezframework.query.Query q = PlayerData.queryBuilder().whereEquals("coins", 42).limit(10).build();
